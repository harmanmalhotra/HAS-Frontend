import React from 'react';
import Form from 'react-bootstrap/Form'
import Col from 'react-bootstrap/Col';
import Row from 'react-bootstrap/Row';
import Button from 'react-bootstrap/Button';
import WardenHeader from '../../WardenHeader';
import '../../css/Style.css';
import { useState, useEffect, useMemo } from "react";
import { useLocalState } from '../../../../util/useLocalStorage';
import {useLocation} from 'react-router-dom';
import { Snackbar } from '@blotoutio/ui-kit';
import {Wrapper} from '../../../admin/hostel/style';

const EditRoom = () => {
    
    const location = useLocation();
    const [snackbar, setSnackbar] = useState({
        variant: 'success',
        message: '',
      });
    const [jwt, setJwt] = useLocalState("", "jwt");

    const [roomStatus, setRoomStatus] = useState("⬇️ Select Room Status ⬇️");
    const [roomType, setRoomType] = useState("⬇️ Select a Room Type ⬇️");
    let handleRoomTypeChange = (e) => {
        setRoomType(e.target.value)
      }

    const[roomTypes,setRoomTypes]=useState([]);
    const [hasLoaded, setHasLoaded] = useState();
    const [roomtypeError, setRoomTypeError] = useState(false);
    const [roomstatusError, setRoomStatusError] = useState(false);
    useEffect(()=>{
        fetch('/warden/viewRoomTypes',{
            method:"GET",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${jwt}`
            }, 
            })
            .then(response => Promise.all([response.json()]))
            .then(([body]) => {
                setRoomTypes(body);
                setHasLoaded(true);
             });
            
    },[]);

    function sendEditRoomRequest () {

        if(roomType === "⬇️ Select a Room Type ⬇️" && roomStatus === "⬇️ Select Room Status ⬇️") {

            setRoomTypeError(true);
            setRoomStatusError(true);
            
            return;
        }else  if(roomType === "⬇️ Select a Room Type ⬇️") {
              setRoomTypeError(true);
              setRoomStatusError(false);
           
            return;
        }else  if(roomStatus === "⬇️ Select Room Status ⬇️") {
            setRoomTypeError(false);
            setRoomStatusError(true);
            
            return;
        }


        fetch(`/warden/editRoom/${location.state.id}, ${roomType}, ${roomStatus}`,{
            method:"PUT",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${jwt}`
            }, 
        })
        .then((response) => {
          if (response.status === 200)
            return Promise.all([response.status, response.text()]);
            else {
                return Promise.reject("Invalid Update Details attempt");
            }
        })
        .then(([status, body]) => {
          //  alert(body);
          setSnackbar({
            message: body,
            variant: 'success',
          });
          window.location.href = "/warden/viewRooms";
        })
        .catch((message) => {
       //   alert(message);
       setSnackbar({
        message: message,
        variant: 'success',
      });
        });
    }

    return (
        hasLoaded 
            ?
        <>
            <WardenHeader />
            <Wrapper>
            <div class=" shadow mb-5 bg-white rounded">
                {/* <section className='d-flex justify-content-between'> */}
                    {/* <div className="left_data mt-2 p-3" style={{ width: "100%" }}> */}
                    <div class="container-fluid">
                      
                     <form>
                        <div class="form-group mb-3">
                        <h5 className='mb-3'><b>Edit Room</b></h5>
                            <label for="exampleFormControlInput1">Room Status</label>
                           
                            <select class="form-control" id="exampleFormControlSelect1" onChange= {(e) => setRoomStatus(e.target.value)}> 
                            
                            <option value="⬇️ Select a Room Status ⬇️"> -- Select a Room Status -- </option>
                                <option value="UnBlocked">UnBlocked</option>
                                <option value="Blocked">Blocked</option>

                                
                            </select>
                            {roomstatusError && <span style={{color: 'red'}}> <p>Room Type should not be empty</p></span>}
                           

                            
                        </div>
                         <div class="form-group mb-3">
                             <label for="exampleFormControlSelect1">Select Room Type</label>
                            <select class="form-control" id="exampleFormControlSelect1" onChange={handleRoomTypeChange}> 
                                <option value="⬇️ Select a Room Type ⬇️"> -- Select a Room Type -- </option>
                                 {roomTypes.map(type => (
                                    <option key={type.roomTypeId} value= {type.roomTypeId}> 
                                        Room Name - {type.roomName},
                                        Room Fee - {type.roomFee}, 
                                        Room Capacity - {type.roomCapacity}
                                        
                                    </option>
                                ))}
                            </select>
                            {roomtypeError && <span style={{color: 'red'}}> <p>Room Type should not be empty</p></span>}
                           
    
                        </div>
                        <div className='mb-3'>
                    <center>   <Button id="sumbit" variant="primary" className='col-lg-6' style={{ background: "#37387a" }} type="button" onClick={() => sendEditRoomRequest()}>
                           Edit Room
                       </Button></center> 
                        </div>
                        
 
</form>
</div>
           </div>
            {snackbar && snackbar.message && (
          <Snackbar
            message={snackbar.message}
            variant={snackbar.variant}
            onClose={() => setSnackbar(null)}
          />
        )}
      </Wrapper>
             </>
             : 
             <p>Loading...</p>
    )
}

export default EditRoom;



