...
const AdminHostelEdit = () => {
  ...
  const fetchData = () => {
    ...
    fetch('/admin/viewHostels1', {
      ...
      .then((data) => {
        console.log(data)
        const hostel = data.find((ele) => ele.hostelId === parseInt(hostelId))
        console.log(hostel)
        console.log(typeof(hostelId))
        const hostelType=hostelTypes.find((ele) => ele.value === hostel.hostelType)
        const hostelStatus=hostelStatuses.find((ele) => ele.value === hostel.hostelStatus)
  ...
  const isValid = () => {
    ...
    if (form.warden.value === 'new-warden') {
      if (!form.newWarden.email || form.newWarden.email.length >= 100 || validateEmail(form.newWarden.email) == false) {
